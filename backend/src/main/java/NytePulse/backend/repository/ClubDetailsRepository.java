...
public interface ClubDetailsRepository extends JpaRepository<ClubDetails, Long> {
    ...
    @Query("SELECT cd FROM ClubDetails cd JOIN User u ON cd.username = u.username " +
            "WHERE LOWER(cd.name) LIKE LOWER(CONCAT('%', :name, '%')) " +
            "AND u.isDeleted = false AND u.isDeactivated = false")
    Page<ClubDetails> findActiveByName(@Param("name") String name, Pageable pageable);
}
